...
func insert_scan(project_id int, tool_id int, address string) {
	stmt, err := connection.name.Prepare(`
		INSERT INTO project_status 
		VALUES(NULL,?,?,?,NULL,NULL);
	`)
	check_error(err)
	result, err := stmt.Exec(tool_id, project_id, address)
	check_error(err)
	check_result(result)
}
...
func completed_scans(project_id int)(*sql.Rows) {
	stmt, err := connection.name.Prepare(`
		SELECT tool_name, command, arguments, address
		FROM project_status
		INNER JOIN tools
...
func queued_scans(project_id int)(*sql.Rows) {
	stmt, err := connection.name.Prepare(`
		SELECT tool_name, command, arguments, address
		FROM project_status
		INNER JOIN tools
